...
public class RFC2Service {
			...
			public Response get20Populares() {
				
				try {
					
					AlohaTransactionManager tm = new AlohaTransactionManager(getPath());
					
					List<RFC2> rfc2 = tm.get20PopularesApto();
			...
			@Consumes({ MediaType.APPLICATION_JSON })
			@Produces({ MediaType.APPLICATION_JSON })
			public Response get20PopularesHabitacion() {
				
				try {
					
					AlohaTransactionManager tm = new AlohaTransactionManager(getPath());
					
					List<RFC2> rfc2 = tm.get20PopularesHabitacion();
			...
			@Consumes({ MediaType.APPLICATION_JSON })
			@Produces({ MediaType.APPLICATION_JSON })
			public Response get20PopularesVivienda() {
				
				try {
					
					AlohaTransactionManager tm = new AlohaTransactionManager(getPath());
					
					List<RFC2> rfc2 = tm.get20PopularesVivienda();
